# DocuPilot: Interview Preparation Guide (AWS Edition)
...
"I used **Docker** with a multi-stage build to minimize the final image size (using Next.js standalone mode). This ensures the image stays within the **ECR Free Tier (500MB)**. I then used **AWS CodePipeline** and **CodeBuild**. The `buildspec.yml` builds the Docker image for the **linux/amd64** architecture, tags it, and pushes it to ECR. This triggers a rolling update in ECS, ensuring zero-downtime deployments."
